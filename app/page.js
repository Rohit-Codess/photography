  ...
  try {
    ...
    return (
      <div className="flex min-h-screen flex-col items-center bg-white text-black">
        ...
        {/* Hero Slider Section */}
        {allPortfolios.length > 0 && <HeroSlider portfolios={allPortfolios} />}
        ...
        <section className="w-full max-w-6xl px-6 py-16 md:py-24">
          ...
          {/* Latest Work Grid */}
          {latestPortfolios.length > 0 && <LatestWorkGrid portfolios={latestPortfolios} />}
    ...
    // Return a fallback page if data fetch fails
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-white text-black px-6">
        ...
        <Link href="/gallery" className="inline-block bg-black text-white font-semibold py-3 px-8 text-sm tracking-widest uppercase transition-colors duration-300 hover:bg-gray-800">
          View Gallery
        </Link>
      </div>
    );
